...
sekiro_shadows_die_twice_location = {}
sekiro_shadows_die_twice_location.__index = sekiro_shadows_die_twice_location
...
function sekiro_shadows_die_twice_location.new(name)
    local self = setmetatable({}, sekiro_shadows_die_twice_location)
    if name then
...
function sekiro_shadows_die_twice_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = sekiro_shadows_die_twice_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function sekiro_shadows_die_twice_location:connect_two_ways(exit, rule)
...
function sekiro_shadows_die_twice_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function sekiro_shadows_die_twice_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function sekiro_shadows_die_twice_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function sekiro_shadows_die_twice_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function sekiro_shadows_die_twice_location:accessibility()
...
-- 
function sekiro_shadows_die_twice_location:discover(accessibility, keys)
...
entry_point = sekiro_shadows_die_twice_location.new("entry_point")
